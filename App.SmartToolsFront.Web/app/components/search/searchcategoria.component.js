...
var SearchCategoriaComponent = /** @class */ (function () {
    ...
    SearchCategoriaComponent.prototype.addToCart = function (p) {
        var _this = this;
        this.loaderService.display(true);
        this.productosService.getStockProducto(p.CodProd).subscribe(function (res2) {
            _this.loaderService.display(false);
            var stock = res2["PrecioVta"];
            if (stock > 0) {
                _this.carritoService.addProduct(p);
            }
            else {
                _this.notifyService.warning('Producto sin stock.');
            }
        }, function (err) { _this.loaderService.display(false); });
    };
